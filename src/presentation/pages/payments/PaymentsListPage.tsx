...
import { PaymentsBI } from "./PaymentsBI";
...
export const PaymentsListPage = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (p) =>
          ...
          p.ServicioAsignado?.Tecnico?.Usuario?.nombre
            ?.toLowerCase()
            .includes(searchTerm.toLowerCase())
      );
    }
  ...
  }, [pagos, selectedEstado, selectedMetodo, searchTerm]);
  ...
  const exportarPDF = () => {
    ...
    const tableData = filteredPagos.map((p) => [
      p.id_pago,
      `${
        p.ServicioAsignado?.SolicitudServicio?.Cliente?.Usuario?.nombre || "N/A"
      } ${
        p.ServicioAsignado?.SolicitudServicio?.Cliente?.Usuario?.apellido || ""
      }`,
      `${p.ServicioAsignado?.Tecnico?.Usuario?.nombre || "N/A"} ${
        p.ServicioAsignado?.Tecnico?.Usuario?.apellido || ""
      }`,
    ...
    autoTable(doc, {
      startY: 58,
      head: [
        ["ID", "Cliente", "Técnico", "Monto", "Método", "Estado", "Fecha"],
      ],
      body: tableData,
      styles: { fontSize: 8 },
      ...
      <div className="p-6 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-indigo-100/70 dark:border-slate-800">
        ...
        <div className="flex gap-4 mb-6 flex-wrap">
          <select
            ...
            className="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-slate-800 dark:text-gray-100"
          >
          ...
          <input
            ...
            className="flex-1 px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-slate-800 dark:text-gray-100"
          />
        </div>
            ...
            <tbody>
              {filteredPagos.length === 0 ? (
                ...
                filteredPagos.map((pago) => (
                  <tr
                    key={pago.id_pago}
                    className="border-b dark:border-slate-700 hover:bg-indigo-50/50 dark:hover:bg-slate-800 transition"
                  >
                    <td className="px-4 py-3 font-medium">#{pago.id_pago}</td>
                    <td className="px-4 py-3">
                      {
                        pago.ServicioAsignado?.SolicitudServicio?.Cliente
                          ?.Usuario?.nombre
                      }{" "}
                      {
                        pago.ServicioAsignado?.SolicitudServicio?.Cliente
                    ...
                    <td className="px-4 py-3">
                      Bs.{" "}
                      {parseFloat(pago.comision_empresa.toString()).toFixed(2)}
                    </td>
                    <td className="px-4 py-3">
